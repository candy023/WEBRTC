---
name: webrtc-bugfix
description: Vue + SkyWay の WebRTC アプリで、join, leave, publish, subscribe, late join, remote stream 表示, screen share, background blur, RNNoise まわりの不具合を最小差分で修正するときに使う。広いリファクタはしない。
---
...
# 特に見る症状
## 1. あとから入室した参加者が、すでに配信中の参加者の映像・音声を見られない
以下を順に確認する。
1. join 後に `subscribeExisting()` が必ず走っているか
2. `room.publications` に既存 publication が見えているか
3. self publication の除外条件が強すぎて他人の publication まで落としていないか
4. `receivedPublicationIds` などの重複防止が早すぎる段階で初回受信を潰していないか
5. `onStream(stream, pub)` までは到達しているか
6. `attachRemoteStream()` の attach 先コンテナが存在するか
7. video/audio 判定で UI 側に落としていないか
8. `play()` 失敗を握りつぶして見逃していないか

## 2. late join で既存参加者だけ見えない
- `room.publications` の走査漏れを疑う
- `subscribeExisting()` の callback 実行順を疑う
- join 後の member 確定前後の race condition を疑う
- 新規配信通知の経路は正常でも、既存購読の経路だけ落ちていないか分けて確認する
...
4. 無関係な UI や命名整理はしない
5. join / leave / 再入室 / late join の観点で確認する
