...
description: DataDog/dtln-rs を使って、Web Audio API + AudioWorklet + WASM ベースの音声ノイズ抑制を SkyWay の送話 publish 経路へ最小差分で導入・調整するときに使う。現行 repo では useRoomSession.js が room join/leave の本体であり、DTLN の ready 到達、processedTrack publish、fallback、bypass/level diagnostics、音量低下調査と最小修正までこの Skill の責務に含める。配布は worklet / glue / wasm の分離配布を第一選択とし、single-file は AudioWorklet 環境差異で外部 wasm 取得が成立しない場合の fallback 戦術として扱う。
---
...
### bypass 調査の原則
- `DTLN_BYPASS_MODE` は調査専用とする
- bypass で正常なら publish 経路は正常、DTLN 処理側が原因と判断する
- bypass でも小さいなら publish 経路や別段を疑う
- bypass を恒久機能として残さない
...
- gain を大きく上げても十分改善しない場合がある
- 最有力原因は次の組み合わせ
  1. `processedTrack` の実効レベル低下
  2. `autoGainControl: false`
  3. DTLN が弱い声を強く抑圧している
- 補助要因として `sampleRate: 16000` 固定による体感上のこもり・明瞭度低下がある
- remote 再生側は二次候補として扱う

### bypass / diagnostics
- `DTLN_BYPASS_MODE` により DTLN 処理を通さず input をそのまま output に流して確認できる
- `dtln-levels` により input / output の RMS、peak、min/max、near-zero 状態を追える
- `denoiseBlockComparison` により 512 サンプル単位の前後比較ができる
...
### 7. `none` フォールバックを明示する
`dtln` 初期化失敗、非対応、ready timeout、処理不能時は通常マイク経路へ戻す。
...
- `outputNearZero`
- `denoiseBlockComparison`

### 最有力原因の判定
- `processedVsRawDb` が大きくマイナス
  - DTLN 後のレベル低下が主因
- `outputNearZero: true` が継続
  - DTLN 出力がほぼ無音
- 大声時だけ output が上がる
  - DTLN が弱い声を強く抑圧している
- `input.rms` 自体がかなり低い
  - AGC off による入力不足が疑わしい
- remote 側 volume / sink / muted
  - 二次候補

### bypass の使い方
- bypass ON で音量が正常
  - publish 経路は正常、DTLN 処理側が原因
- bypass ON でも小さい
  - publish 経路か別段を疑う
...
### 第一候補
- まず bypass と diagnostics で原因を固定する
- 問題が DTLN 処理側だと確認できたら `autoGainControl: true` を戻して再計測する
- 必要なら `echoCancellation: true` も戻す
- `noiseSuppression` は DTLN と二重にならないよう慎重に扱う

### 第二候補
- DTLN 後段に `GainNode` を追加する
...
- bypass と diagnostics は調査用であり、恒久仕様として増やしすぎない
...
8. bypass ON/OFF で publish 経路問題か DTLN 問題かを切り分けられる

9. 音量低下修正は diagnostics → AGC → GainNode → 必要時 Compressor の順で最小差分に留まっている
...
- gain を大きくしても声がまだ極端に小さいことがある
- 最有力候補は processedTrack のレベル低下、AGC off、DTLN の弱い声抑圧
- bypass と diagnostics で publish 経路問題か DTLN 問題かを切り分けたい
...
2. bypass ON/OFF で publish 経路問題か DTLN 問題かを切り分ける
3. 最有力原因を固定する
4. 必要最小限で AGC / GainNode / 必要時 Compressor の順に調整する
5. DTLN 失敗時は通常マイクへフォールバックする
6. 分離配布を第一選択にする
7. 分離配布が成立しない場合のみ single-file fallback を使う
8. 既存 API shape と UI state を壊さない
