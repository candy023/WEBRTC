...
### 2-5. 共有画面と参加者カメラは別責務で扱う
- 共有画面と参加者カメラを同じ一覧に戻さない
- 共有画面の主表示責務と、参加者カメラの補助表示責務を混ぜない

### 2-6. モバイルでは主要映像を横スクロール前提にしない
- モバイル UI では、主要映像が横スクロールしないと見えない構成にしない
- ユーザーが横へスワイプしないと主要情報へ到達できない構成は避ける

### 2-7. 共有画面がある場合は共有を主表示にする
- 共有画面が存在する場合、共有画面が主表示である
- 参加者カメラは補助表示であり、共有画面の代わりにならない
