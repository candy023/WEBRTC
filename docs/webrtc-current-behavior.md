...
### UI レイアウト
- 共有画面と参加者カメラは同じ一覧ではなく、別責務として扱う
- 共有画面が存在する場合は、共有画面を主表示として扱う
- 参加者カメラは共有画面とは別の帯域で表示する
- 現在の UI 問題は接続不具合ではなく、端末ごとの見え方や優先順位の問題として扱う
...
- 共有画面がある場合は共有画面を主表示とし、参加者カメラは補助表示とする
- 補助表示は横フィルムストリップでもよい

### モバイルの UI 方針
- モバイルでは、主要映像が横スクロールしないと見えない構成にしない
- モバイルでは、共有画面がある場合は共有画面を最優先で上部に配置する
- モバイルでは、参加者カメラを横フィルムストリップより縦積みまたはグリッド表示で優先する
- 「見えていないだけ」を接続不具合と誤認しやすい UI を避ける
...
### レスポンシブ UI の扱い
レスポンシブ UI の修正は、接続ロジックや購読ロジックの問題と混同しない。
画面共有、参加者カメラ、プレースホルダの見え方を直す場合は、まず `WebRTC_UI.vue` 側のレイアウト責務として扱う。
...
- モバイル時の参加者カメラを 1 列優先にするか 2 列グリッド優先にするか
